...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            double smth = sin(pointX) + cos(pointY + pointZ);
    ...
    void doTimeStep(double t) {
        ...
        for(unsigned int i = 0; i < nodes.size(); i++) {
            double pointX = nodes[i].x;
            double pointY = nodes[i].y;
            double pointZ = nodes[i].z;
            nodes[i].vx = (pointY + 31.2) * cos(t/2) / 500;
            nodes[i].vy = -(pointX + 1.0) * cos(t/2) / 500;
            if ((pointY < -35) and (pointY > -42) and (pointZ < 8) and (pointZ > 2)) {
                nodes[i].vx +=(-pointY - 35) * cos(t) / 150;
            }
            nodes[i].move(t);
        }
    }
    ...
    void snapshot(unsigned int snap_number) {
        ...
        string fileName = "godzilla_steps/godzilla-step-" + std::to_string(snap_number) + ".vtu";
...
int main()
{
    std::filesystem::create_directory("godzilla_steps");
    ...
    gmsh::initialize();
    gmsh::model::add("godzilla");

    // Считаем STL
    try {
        gmsh::merge("../godzilla.stl"); 
    } 
    catch(...) {
    ...
    for(unsigned int step = 1; step < 128; step++) {
